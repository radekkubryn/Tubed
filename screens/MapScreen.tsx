...
const MapScreen: React.FC<Props> = ({ progress, onLevelSelect, onBack }) => {
  ...
  useEffect(() => {
    // Add a small delay to ensure layout is fully rendered, especially for reverse-flex containers on mobile
      ...
      if (scrollRef.current) {
        ...
        if (unlockedNode) {
          unlockedNode.scrollIntoView({ behavior: 'smooth', block: 'center' });
            ...
            // Fallback if completed everything or node not found
  ...
  // Generate curvy path positions
  // We'll simulate a sine wave path using left margin percentages
  ...
  return (
    <div className="flex flex-col h-full bg-slate-900">
      <div className="p-4 bg-slate-800 shadow-md flex items-center z-10 shrink-0">
        ...
        <h1 className="text-xl font-bold text-white">Mapa Poziomów</h1>
      ...
      <div 
        ref={scrollRef}
        className="flex-1 overflow-y-auto relative p-8 bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] bg-fixed"
      >
        <div className="relative flex flex-col-reverse items-center min-h-full py-10 gap-12">
            {/* Dashed Path Line - Simple central line implementation for robustness */}
            ...
            {levels.map((level) => {
                ...
                // Offset calculation for "Snake" effect
                // sine wave logic: Math.sin(level) * offset
                ...
                return (
                    <div 
                        ...
                        className={`
                            ...
                            border-4 shadow-lg transition-transform hover:scale-110 cursor-pointer shrink-0
                    ...
                    >
                        ...
                        {isCurrent && (
                            <div className="absolute -top-8 bg-yellow-500 text-slate-900 text-[10px] font-bold px-2 py-1 rounded shadow animate-bounce whitespace-nowrap">
                                GRAJ
                            </div>
                        )}
                    </div>
