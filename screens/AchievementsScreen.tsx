...
import { UserProgress, ACHIEVEMENTS_DATA, Achievement, Language } from '../types';
...
const AchievementsScreen: React.FC<Props> = ({ onBack }) => {
  ...
  const [activeTab, setActiveTab] = useState<'all' | 'progress' | 'mechanic' | 'skill' | 'collection' | any>('all');
    ...
    <div className="flex flex-col h-full bg-slate-900 text-white">
      <div className="p-4 bg-slate-800 shadow-md flex flex-col gap-2 z-10 shrink-0">
         <div className="flex items-center">
            ...
            <h1 className="text-xl font-bold text-yellow-500 uppercase">{t('trophies', lang)}</h1>
         ...
         <div className="flex flex-wrap gap-2 justify-center pt-2">
             {['all', 'progress', 'mechanic', 'skill', 'collection'].map(tab => (
                 ...
                 >
                     {tab === 'all' ? t('all', lang) : tab === 'progress' ? t('levels', lang) : tab === 'mechanic' ? t('mechanics', lang) : tab === 'skill' ? t('skill', lang) : t('collection', lang)}
                 </button>
             ))}
         </div>
      </div>
                  ...
                  <div key={ach.id} className={`relative p-3 rounded-xl border flex gap-3 ${isClaimed ? 'bg-slate-800/40 border-slate-700 opacity-60' : (isCompleted ? 'bg-indigo-900/40 border-yellow-500' : 'bg-slate-800 border-slate-700')}`}>
                      ...
                      <div className="flex-1 min-w-0">
                          <div className="flex justify-between items-start">
                              <h3 className={`font-bold text-sm truncate ${isCompleted && !isClaimed ? 'text-yellow-400' : 'text-slate-200'}`}>{ach.title[lang]}</h3>
                              {isClaimed && <span className="text-xs text-green-400 font-bold uppercase">{lang === 'pl' ? 'ODEBRANO' : 'CLAIMED'}</span>}
                          </div>
                          <p className="text-xs text-slate-400 truncate">{ach.description[lang]}</p>
                      ...
                      <div className="flex flex-col items-center justify-center border-l border-slate-700 pl-3 min-w-[70px]">
                          {isClaimed ? (
                              ...
                              <button 
                                ...
                                className="w-full py-1 px-2 bg-yellow-500 hover:bg-yellow-400 text-slate-900 text-xs font-bold rounded animate-pulse shadow-[0_0_10px_rgba(234,179,8,0.5)] uppercase"
                              >
                                  {t('claim', lang)}
                              ...
                              <div className="flex flex-col items-center opacity-70">
                                   <span className="text-xs font-bold text-slate-400 uppercase">{lang === 'pl' ? 'Nagroda' : 'Reward'}</span>
